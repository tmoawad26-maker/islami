...
class SuraListView extends StatelessWidget {
  const SuraListView({super.key, required this.filterList});
  ...
  Widget build(BuildContext context) {
    ...
    return Visibility(
      ...
      replacement: Center(
        child: Text('No sura item', style: AppTextStyles.textStyle16Bold),
      ),
      child: ListView.separated(
        ...
        itemBuilder: (context, index) {
          return InkWell(
            onTap: () async {
              CachedData.saveMostRecentlySura(
                SuraModel.suraModelList[index].index,
              );
            ...
            child: CustomSuraSearch(suraModel:  filterList[index]),
          );
        },
